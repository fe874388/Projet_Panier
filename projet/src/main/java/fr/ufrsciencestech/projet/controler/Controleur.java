...
public class Controleur implements ActionListener {
    ...
    private FruitFactory CurrentFruit;
    ...
    public void actionPerformed(ActionEvent e){   //Invoked when an action occurs
        if (((Component) e.getSource()).getName().equals("ComboBox")){
            CurrentFruit = (FruitFactory) vue.getjComboBox().getSelectedItem();
            System.out.println("**- CHANGEMENT DANS LA COMBOBOX Fruit Courant : " + CurrentFruit + " -**");
        }
        if(((Component)e.getSource()).getName().equals("Plus")) {
            FruitFactory fabriqueFruit = new FabriqueFruit();
            vue.getjComboBox().addItem(fabriqueFruit);
    ...
    public void setCurrentFruit(FruitFactory c){
        this.CurrentFruit = c;
    }
    public FruitFactory getCurrentFruit(){
        return CurrentFruit;
    }
